...
```
    [root@localhost src]# vim /etc/init.d/rabbitmq-server
        #!/bin/sh
        #
    ...
    #添加执行权限   
    [root@localhost src]# chmod u+x /etc/init.d/rabbitmq-server
    #配置为开机自启动服务
    [root@localhost src]# ln -s $(which erl) /usr/bin/erl && chkconfig rabbitmq-server on
    [root@localhost src]# 
    [root@localhost src]# 
    [root@localhost src]# 
...
* 设置启动命令：chkconfig rabbitmq-server on
